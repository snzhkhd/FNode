...
static std::shared_ptr<NodeBase> CreateNodeFromTemplate(const NodeTemplate& tpl, const ScriptFile& scriptFile, Vector2 position)
{
    ...
    for (const auto& pt : tpl.ports) {
        ...
        p.type = pt.type;
        p.isArray = false;
        p.isInput = pt.isInput;
        p.isHovered = false;
        p.selectedIndex = 0;
        p.enumValues = pt.enumValues;
...
static std::vector<NodeTemplate> g_NodeTemplates = {
    ...
    { "Add (Float)", true,"Math | Float",
        {
            PortTemplate("A", EVarType::Float, true),
            PortTemplate("B", EVarType::Float, true),
            PortTemplate("Result", EVarType::Float, false),
        }
    },
    ...
    { "Add (Int)", true,"Math | Int",
        {
             PortTemplate("A", EVarType::Int, true),
             PortTemplate("B", EVarType::Int, true),
             PortTemplate("Result", EVarType::Int, false),
        }
    },
    // Boolean AND
    { "AND", true,"Math | Bool",
        {
             PortTemplate("A", EVarType::Bool, true),
             PortTemplate("B", EVarType::Bool, true),
             PortTemplate("Result", EVarType::Bool, false),
        }
    },
    ...
    { "NOT", true,"Math | Bool",
        {
             PortTemplate("Value", EVarType::Bool, true),
             PortTemplate("Result", EVarType::Bool, false),
        }
    },
    ...
    { "Branch", false,"Math | Bool",
        {
             PortTemplate("Condition", EVarType::Bool, true),
             PortTemplate("True", EVarType::EXEC, false),   // используем Int как "placeholder" для exec
             PortTemplate("False", EVarType::EXEC, false)
        }
    },
    { "Messages types", true, "Client | Define", 0,
        {
            PortTemplate("Type", EVarType::Defines, true,
                {
                    "FOMB_GAME",
                    "FOMB_TALK",
                    "FOMB_COMBAT_RESULT",
            ...
            PortTemplate("",EVarType::Int, false)
        }
    },
